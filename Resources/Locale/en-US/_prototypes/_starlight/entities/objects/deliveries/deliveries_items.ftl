ent-MailSyndicateSpamLetter = union leaflet
    .desc = A propaganda leaflet that smells like cheap ink.

ent-RRMailSyndicateSpamLetter = union leaflet
    .desc = A propaganda leaflet that smells like cheap ink.

ent-MailSpamDontYouWantMore = spam letter
    .desc = A letter with a loud headline and a questionable offer.

ent-MailSovietSpamLetter100 = workers' rights leaflet
    .desc = A propaganda pamphlet that smells of burnt matches. It's so thick it won't fit in the fax.

ent-MailSovietSpamLetter50 = { ent-MailSovietSpamLetter100 }
    .desc = { ent-MailSovietSpamLetter100.desc }

ent-MailSovietSpamLetter25 = { ent-MailSovietSpamLetter100 }
    .desc = { ent-MailSovietSpamLetter100.desc }

ent-MailSovietSpamLetter10 = { ent-MailSovietSpamLetter100 }
    .desc = { ent-MailSovietSpamLetter100.desc }

ent-MailSovietSpamLetter30Faxable = { ent-MailSovietSpamLetter100 }
    .desc = A thin propaganda pamphlet that smells of burnt matches.
